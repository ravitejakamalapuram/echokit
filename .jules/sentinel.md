...
## 2025-02-23 - DOM Purifier XSS Bypass via URL Attributes
**Vulnerability:** The HTML sanitizer (`extension/shared/sanitize.js`) allowed dangerous `javascript:` URIs through attributes like `action` (on forms) and `xlink:href` (on SVGs), enabling Cross-Site Scripting (XSS).
**Learning:** When sanitizing potentially untrusted DOM elements for dangerous URI schemes, standard checks on `href` and `src` are insufficient. Other URL-accepting attributes can also execute JavaScript.
**Prevention:** Ensure that checks for dangerous URL prefixes (like `javascript:`) cover all URL-accepting attributes, explicitly including `action` and `xlink:href`, to prevent XSS bypasses.
