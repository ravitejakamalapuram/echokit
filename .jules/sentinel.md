...
## 2026-07-13 - Prevent DOM Clobbering Vulnerability in all DOM Methods
**Vulnerability:** The codebase had many instances where DOM methods like `el.getAttribute` and `el.setAttribute` were used directly on the element object. This is vulnerable to DOM clobbering, where an attacker could inject elements (e.g., `<input name="getAttribute">`) that would override these methods and cause exceptions or unexpected behavior, potentially leading to XSS or other vulnerabilities.
**Learning:** Always use `Element.prototype` methods directly (e.g., `Element.prototype.getAttribute.call(el, 'name')`) to securely interact with elements and avoid clobbering risks. This should be applied not just in the sanitizer, but everywhere DOM attributes or properties are accessed on potentially user-controlled or dynamically generated content.
**Prevention:** Replaced all direct calls to `el.getAttribute`, `el.removeAttribute`, `el.setAttribute`, and property access like `el.tagName` with their `Element.prototype` equivalents (e.g., `Element.prototype.getAttribute.call(el, 'attr')` and `Object.getOwnPropertyDescriptor(Element.prototype, 'tagName').get.call(el)`).
