...
## 2026-07-25 - Prevent DOM XSS via action/xlink:href attributes
**Vulnerability:** The HTML sanitizer only checked `href` and `src` attributes for dangerous prefixes (like `javascript:`), omitting other URL-accepting attributes like `action` and `xlink:href`.
**Learning:** XSS bypasses can exploit less common URL-accepting attributes if a sanitizer is not comprehensive.
**Prevention:** Added `action` and `xlink:href` to the prefix validation check in `sanitizeHTML`.
