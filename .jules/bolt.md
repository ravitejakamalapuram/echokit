...
## 2026-08-12 - O(N) Array Filtering and Object Allocation in Row Rendering Loop
**Learning:** Configuration getters that dynamically filter and compute array representations (like `getColumnsForMode`) cause significant garbage collection pressure when called repeatedly inside O(N) row-rendering loops (e.g., `renderInteractionRow`).
**Action:** Memoize or cache the results of configuration getters that return filtered arrays or derived configurations to ensure O(1) performance inside rendering loops.
