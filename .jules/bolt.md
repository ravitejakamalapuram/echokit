...
## 2025-02-18 - URL parsing overhead in tight rendering loops
...
**Action:** Use the \`parseUrl\` utility from \`extension/shared/interaction-helpers.js\`, which utilizes a module-level \`Map\` cache (\`urlCache\`) with a size limit to store and retrieve parsed URLs in O(1) time.
