...
## 2026-06-19 - URL Parsing Bottleneck
**Learning:** Instantiating `new URL()` inside tight loops during UI rendering (like `renderRow` or `groupByDomain`) is a severe performance bottleneck causing O(K*N) impact.
**Action:** Always use the `parseUrl` utility from `extension/shared/interaction-helpers.js`, which utilizes an LRU Map cache to store and retrieve parsed URLs in O(1) time. Do not mutate the cached returned object.
