...
// Cache for hostOf to prevent O(N) new URL() allocations during tight filtering loops
// like visibleInContext.
const MAX_URL_HOST_CACHE_SIZE = 1000;
const urlHostCache = new Map();

function hostOf(url) {
  if (!url) return '';
  if (urlHostCache.has(url)) {
    return urlHostCache.get(url);
  }
  try {
