...
function renderInteractionRow(i) {
  ...
  const path = (() => { try { const u = parseUrl(i.url, location.href); return u ? u.pathname : i.url; } catch { return i.url; } })();
...
function domainOf(url) { try { const u = parseUrl(url, location.href); return u ? u.host || '(local)' : '(unknown)'; } catch { return '(unknown)'; } }
function prettyUrl(url) {
  try { const u = parseUrl(url, location.href); return u ? { path: u.pathname, query: u.search } : { path: url, query: '' }; }
