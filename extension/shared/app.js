...
function filteredInteractions() {
  ...
  // Pre-calculate filter values outside the loop for O(1) checks
  const q = state.search.trim().toLowerCase();

  const reqBodyQuery = features.bodySearch && state.filters.requestBodyContains
    ? state.filters.requestBodyContains.toLowerCase() : null;
  const resBodyQuery = features.bodySearch && state.filters.responseBodyContains
    ? state.filters.responseBodyContains.toLowerCase() : null;

  const reqHeaderName = features.headerSearch ? state.filters.requestHeader.name : null;
  const reqHeaderValue = features.headerSearch ? state.filters.requestHeader.value : null;
  const resHeaderName = features.headerSearch ? state.filters.responseHeader.name : null;
  const resHeaderValue = features.headerSearch ? state.filters.responseHeader.value : null;
  ...
  let results = state.interactions.filter(i => {
    ...
    if (features.multiSelect && state.filters.statusCodes.length > 0) {
      ...
      const matchesBucket = state.statusFilter === '0'
        ? (i.responseStatus || 0) === 0
        : bucket === state.statusFilter;
    ...
    if (state.filters.hasNotes !== null) {
      ...
      if (state.filters.hasNotes ? !hasNotes : hasNotes) return false;
    }
