...
function renderRow(i) {
  ...
  const urlPretty = prettyUrl(i.url, location.href);
...
function renderInteractionRow(i) {
  ...
  const pUrl = parseUrl(i.url);
  const path = pUrl ? pUrl.pathname : i.url;
...
function domainOf(url) {
  const pUrl = parseUrl(url, location.href);
  return pUrl ? (pUrl.host || '(local)') : '(unknown)';
}
// prettyUrl is imported from interaction-helpers.js
