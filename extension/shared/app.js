...
function filteredInteractions() {
  ...
  const reqBodyContains = state.filters.requestBodyContains ? state.filters.requestBodyContains.toLowerCase() : '';
  const resBodyContains = state.filters.responseBodyContains ? state.filters.responseBodyContains.toLowerCase() : '';

  // ⚡ Bolt: Consolidated multiple `.filter()` calls into a single O(N) pass.
  // Impact: Reduces garbage collection overhead and intermediate array creations, improving rendering performance.
  results = results.filter(i => {
    ...
    if (features.bodySearch) {
      if (reqBodyContains && !searchBodyContent(i.requestBody, reqBodyContains)) return false;
      if (resBodyContains && !searchBodyContent(i.responseBody, resBodyContains)) return false;
    }
