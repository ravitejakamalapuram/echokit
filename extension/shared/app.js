...
function sanitizeHTML(html) {
  return DOMPurify.sanitize(html, {
    ADD_ATTR: ['data-testid', 'data-action', 'data-id', 'data-menu', 'data-a', 'data-ce-id', 'data-idx', 'data-key', 'data-orig', 'data-step', 'data-mirror-for', 'data-tab-id', 'role', 'tabindex', 'autocomplete', 'spellcheck']
  });
}
...
function softRenderList() {
  ...
  if (isPopup || !features.sortableColumns) {
    ...
    list.innerHTML = sanitizeHTML(items.length === 0 ? renderEmpty() : grouped.map(renderDomainGroup).join(''));
  } else {
    // Table view
