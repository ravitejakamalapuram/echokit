...
function renderDetail(i, conflicts) {
  ...
  return `
    <div class="ek-detail-head">
      ...
      <button class="ek-close" data-action="close-detail" data-testid="close-detail" title="close details" aria-label="close details">✕</button>
    </div>
            ...
            <div data-testid="headers-list">
              ${Object.entries(overrideHeaders).map(([k, v], idx) => `
                <div class="ek-kv-row">
                  ...
                  <button class="ek-kv-remove" data-action="header-remove" data-id="${i.id}" data-key="${escapeHtml(k)}" title="remove header override" aria-label="remove header override">×</button>
                </div>
              `).join('')}
            </div>
