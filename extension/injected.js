  ...
  function parseGraphQL(body, url) {
    ...
    try {
      ...
      if (q) return {
        ...
        variables: (() => {
          try {
            return JSON.parse(u.searchParams.get("variables") || "{}");
          } catch {
            return {};
          }
        })()
  ...
  (function() {
    ...
    window.addEventListener("message", (ev) => {
      ...
      if (d.type === "echokit:mockIndex") {
        const p = d.payload || {};
        if (p.mocks) {
          state.mockIndex = p.mocks;
          state.blockedKeys = p.blocked || state.blockedKeys;
        } else {
          state.mockIndex = p;
        }
    ...
    function applyResponseTransforms(body, headers, url) {
      ...
      for (const rule of rules) {
        try {
          ...
          else if (rule.action === "remove-header") {
            headers = { ...headers };
          ...
          try {
            if (res) {
              ...
              emit("record", {
                ...
                requestHeaders: modifiedReqHeaders,
                requestBody: reqBody,
    ...
    async function bodyToText(body) {
      ...
      if (typeof FormData !== "undefined" && body instanceof FormData) {
        const o = {};
        for (const [k, v] of body.entries()) o[k] = typeof v === "string" ? v : "[file]";
    ...
    function createFakeMockWS(url, mock) {
      ...
      const fake = {
        ...
        close(code) {
          ...
          try {
            dispatch("close", new CloseEvent("close", { wasClean: true, code: code || 1e3, reason: "" }));
          } catch {
          }
        }
      };
      setTimeout(() => {
        ...
        if (loop && inFrames.length > 0) {
          const dur = (inFrames[inFrames.length - 1]?.t || 1e3) + 1e3;
    ...
    function createFakeMockSSE(url, mock) {
      ...
      setTimeout(() => {
        ...
        if (loop && frames.length > 0) {
          const dur = (frames[frames.length - 1]?.t || 1e3) + 1e3;
          _loopTimer = setInterval(() => {
            if (_closed) {
              clearInterval(_loopTimer);
              return;
    ...
    if (OrigWS) {
      let EchoKitWS = function(url, protocols) {
      ...
      try {
        window.WebSocket = EchoKitWS;
      } catch {
      }
    }
    const OrigES = window.EventSource;
    if (OrigES) {
      let EchoKitES = function(url, init) {
        const ekUrl = String(url);
