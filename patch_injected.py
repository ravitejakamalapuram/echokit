import re
with open('extension/injected.js', 'r') as f:
    content = f.read()

old_func = """  function computeMatchKeys(method, url, body) {
...
new_func = """  function computeMatchKeys(method, url, body) {
    const M = String(method || 'GET').toUpperCase();
...
if old_func in content:
    ...
    with open('extension/injected.js', 'w', encoding='utf-8') as f:
        f.write(content)
    print("Patched extension/injected.js successfully.")
else:
    print("Could not find the target string in extension/injected.js.")
