...
### 🧹 Code Quality & Performance (from 2026-06-27 Engineering Review)

- [ ] **Replace `new URL()` with cached `parseUrl` in loops**
  - **Where**: `background.js` filtering logic, `app.js` rendering
  - **Why**: Severe performance bottleneck with high network traffic
  - **Implementation**: Create LRU cache in `interaction-helpers.js`

- [ ] **Combine sequential array `.filter()` calls into single passes**
  - **Where**: Background interaction filtering, UI rendering
  - **Why**: O(K*N) performance overhead
  - **Implementation**: Rewrite filter chains as single-pass loops

- [ ] **Remove production `console.log` calls**
  - **Where**: `app.js`, `background.js`
  - **Why**: Violates DEVELOPMENT_RULES.md production path rules
  - **Implementation**: Audit and remove all console.log not wrapped in comment explanations

- [ ] **Wrap all JSON.parse and network/storage calls in try-catch**
  - **Where**: All storage reads, network payloads, import functions
  - **Why**: Missing error boundaries cause silent failures
  - **Implementation**: Systematic audit and error handling

- [ ] **Create central event delegation system**
  - **Where**: `app.js` scattered addEventListener blocks
  - **Why**: Fragmented listener setup, hard to maintain
  - **Implementation**: Single delegated click handler at root

- [ ] **Fix DOM XSS in URI validation logic**
  - **Where**: `sanitize.js` control character handling
  - **Why**: Security vulnerability - control chars bypass `javascript:` prefix checks
  - **Implementation**: Strip control chars before URI validation

- [ ] **Remove magic numbers across codebase**
  - **Where**: Hardcoded delays, timeouts, buffer sizes
  - **Why**: Violates code quality standards
  - **Implementation**: Extract to named constants

### 🏗️ Architecture Improvements

- [ ] **Refactor `background.js` into service modules**
  - **Goal**: Break 1800+ line god file into focused services
  - **Modules**: StorageService, NetworkService, DNRService, LicenseService
  - **Approach**: Incremental extraction, maintain backward compatibility
